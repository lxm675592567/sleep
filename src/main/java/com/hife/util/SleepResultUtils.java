...
public class SleepResultUtils {
    ...
    public static JSONObject getSleepResult(JSONObject json) throws ParseException {
        ...
        getchushishushui(listTimesm, shushui, shushuiList);
        ...
        List<List<Object>> smlx = getSmlx(qujianlist, ssList, zdssList, qsList, qxList, ksydList, hxwlList, smlxResult,date);
...
//        object.put("hxwlList",hxwlList);//呼吸紊乱
    ...
    private static List<List<Object>> getSmlx(List<List<Integer>> qujianlist, List<List<Object>> ssList, List<List<Object>> zdssList, List<List<Object>> qsList, List<List<Object>> qxList, List<List<Object>> ksydList, List<List<Integer>> hxwlList, List<List<Object>> smlxList, Date date) {
        //先将五值依次遍历
...
//        for (int i = 1; i < shushuiNewList.size(); i++) {
//            //用i前坐标值-上一个最后坐标值 小于5分钟删除上一个
//            List sqianList = (List) shushuiNewList.get(i-1).get(shushuiNewList.get(i-1).size()-1).get(0);//上一个最后值
//            List qianList = (List) shushuiNewList.get(i).get(0).get(0);
//            List houList = (List) shushuiNewList.get(i).get(shushuiNewList.get(i).size()-1).get(0);
//            int sqianzb = (int) sqianList.get(1);
//            int qianzb = (int) qianList.get(0);
//            int jincha = abs(qianzb - sqianzb);//相近减
//
//            List sqiansList = (List) shushuiNewList.get(i-1).get(shushuiNewList.get(i-1).size()-1).get(0);//上一个shou值
//            int sqianszb = (int) sqianList.get(0);
//            int houzb = (int) qianList.get(1);
//            int yuancha = abs(sqianszb - houzb);
//            if (jincha<300 && yuancha<300){
//                shushuiNewList.get(i-1).remove(shushuiNewList.get(i-1).size()-1);
//            }
//            if (shushuiNewList.get(i-1).size()==0){
//                shushuiNewList.remove(i-1);
//            }
//        }
    }
    ...
    private static void getchushishushui(List<List<Integer>> listTimesm, List<List<Object>> shushui, List<List<List<Object>>> shushuiList) {
